...
public:
	Connection(unsigned int dep_id, unsigned int arr_id, unsigned int dep_time, unsigned int arr_time) {
		this->dep_id = dep_id;
		this->arr_id = arr_id;
	...
	friend std::ostream& operator<< (std::ostream& os, const Connection& connection) {
		os << connection.dep_id << " [" << connection.dep_time << "] -> " << connection.arr_id << " [" << connection.arr_time << "]";
		return os;
	}
};
#endif